...
// Compute for shadow if hasShadow is true.
    ...
    //***********************************************
    Vector3d nearestPoint = uRay.pointAtParam(nearest_t);
    for (int i = 0; i < scene.numPtLights; i++) {
        Vector3d L = scene.ptLight[i].position - nearestPoint;
        Ray tempRay = Ray(nearestPoint, L.unitVector());
        bool isBlock = false;
        
        // Shadow Mode is on
        if (hasShadow) {
            for (int j = 0; j < scene.numSurfaces; j++) {
                if (scene.surfacep[j]-> shadowHit(tempRay, DEFAULT_TMIN, L.length())) {
                    // Remember if it is being blocked
                    isBlock = true;
                    break;
                }
            }
            if (!isBlock) {
                // If it is not blocked, add to the result
                result+= computePhongLighting(tempRay.direction(), N, V, *nearestHitRec.mat_ptr, scene.ptLight[i]);
            }
        } else {
            // Shadow Mode if off
            result+= computePhongLighting(tempRay.direction(), N, V, *nearestHitRec.mat_ptr, scene.ptLight[i]);
...
// Add to result the reflection of the scene.
    ...
    if (reflectLevels == 0) {
        ...
        Ray newRay = Ray(nearestPoint, mirrorReflect(-uRay.direction(), N));
        
        // Recursive
        return result + nearestHitRec.mat_ptr->k_rg * Raytrace::TraceRay(newRay, scene, reflectLevels - 1, hasShadow);
    }
}

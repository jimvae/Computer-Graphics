...
const char rustTexFile[] = "images/rust.jpg";
const char windowTexFile[] = "images/window.jpg";
...
void MakeReflectionImage( void )
{
    ...
    float reflectionHeight = (2 * TABLETOP_Z) - eyePos[2]; // Reflected Height of the actual eye via table top
    glMatrixMode(GL_MODELVIEW);
    glLoadIdentity();
    gluLookAt(eyePos[0], eyePos[1], reflectionHeight, eyePos[0], eyePos[1], eyePos[2], 1, 0, 0);
...
//    Concatenate the execution path to the image paths so that fopen() works.
    ...
    std::string rustPath = execPath + "/" + std::string(rustTexFile);
    std::string windowPath = execPath + "/" + std::string(windowTexFile);
...
// This texture object is for the spots texture map.
    ...
    // This texture object is for the rust texture map.

        glGenTextures( 1, &rustTexObj );
        glBindTexture( GL_TEXTURE_2D, rustTexObj );
        glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT );
        glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT );
        glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR );
        glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR );

        if ( ReadImageFile( rustPath.data(), &imageData,
                            &imageWidth, &imageHeight, &numComponents ) == 0 ) exit( 1 );
        if ( numComponents != 3 )
        {
            fprintf( stderr, "Error: Texture image is not in RGB format.\n" );
    ...
    // This texture object is for the window texture map.

        glGenTextures( 1, &windowTexObj );
        glBindTexture( GL_TEXTURE_2D, windowTexObj );
        glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT );
        glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT );
        glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR );
        glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR );

        if ( ReadImageFile( windowPath.data(), &imageData,
                            &imageWidth, &imageHeight, &numComponents ) == 0 ) exit( 1 );
        if ( numComponents != 3 )
        {
            fprintf( stderr, "Error: Texture image is not in RGB format.\n" );
...
double wingOuterY = 0.2;
double wingTopY = 0.15;
double wingMidY = 0.05;
double wingBottomY = 0.0;
double wingTopX = 0.4;
double wingMidX = 0.3;
double wingBottomX = 0.0;
double wingTopZ = 0.02;
double wingMidZ = 0.005;
double wingBottomZ = 0.0;

double pilotTopY = 0.3;
double pilotMidY = 0.2;
double pilotBottomY = -0.005;
double pilotBackWidth = 0.05;
double pilotFrontWidth = 0.03;
double pilotTopZ = 0.1;
double pilotMidZ = 0.05;
double pilotBottomZ = 0.0;

double bodyOuterY = 0.6;
double bodyTopY = 0.3;
double bodyMidY = 0.2;
double bodyBottomY = -0.005;
double bodyOuterWidth = 0.01;
double bodyBackWidth = 0.05;
double bodyFrontWidth = 0.03;
double bodyTopZ = 0.1;
double bodyMidZ = 0.05;
double bodyBottomZ = 0.0;

double upperWingHeight = 0.04;
double lowerWingHeight = 0.01;
double xPlaneLocation = 0.7;
double yPlaneLocation =  -0.7;
double planeHeight = TABLETOP_Z * 1.3;
...
void DrawRightWing()
{
    ...
    // +z
    glNormal3f( 0.0, 0.0, 1.0 ); // Normal vector.
    SubdivideAndDrawQuad( 24, 24,  0.0, 0.0, wingMidX, wingMidY, wingTopZ,
                                  1.0, 0.0, wingBottomX, wingTopY, wingTopZ,
                                  1.0, 1.0, wingBottomX, wingBottomY, wingTopZ,
                                  0.0, 1.0, wingMidX, wingBottomY, wingTopZ );
    
    // -z
    glNormal3f( 0.0, 0.0, -1.0 ); // Normal vector.
    SubdivideAndDrawQuad( 24, 24,  0.0, 0.0, wingMidX, wingMidY, wingBottomZ,
                                  1.0, 0.0, wingMidX, wingBottomY, wingBottomZ,
                                  1.0, 1.0, wingBottomX, wingBottomY, wingBottomZ,
                                  0.0, 1.0, wingBottomX, wingTopY, wingBottomZ );
    
    // +x
    glNormal3f( 1.0, 0.0, 0.0 ); // Normal vector.
    SubdivideAndDrawQuad( 24, 24,  0.0, 0.0, wingMidX, wingMidY, wingTopZ,
                                  1.0, 0.0, wingMidX, wingBottomY, wingTopZ,
                                  1.0, 1.0, wingMidX, wingBottomY, wingBottomZ,
                                  0.0, 1.0, wingMidX, wingMidY, wingBottomZ );
    
    // -x
    glNormal3f( -1.0, 0.0, 0.0 ); // Normal vector.
    SubdivideAndDrawQuad( 24, 24,  0.0, 0.0, wingBottomX, wingTopY, wingTopZ,
                                  1.0, 0.0, wingBottomX, wingTopY, wingBottomZ,
                                  1.0, 1.0, wingBottomX, wingBottomY, wingBottomZ,
                                  0.0, 1.0, wingBottomX, wingBottomY, wingTopZ );
    
    // +y
    glNormal3f( 1.0, 0.0, 0.0 ); // Normal vector.
    SubdivideAndDrawQuad( 24, 24,  0.0, 0.0, wingBottomX, wingTopY, wingTopZ,
                                  1.0, 0.0, wingMidX, wingMidY, wingTopZ,
                                  1.0, 1.0, wingMidX, wingMidY, wingBottomZ,
                                  0.0, 1.0, wingBottomX, wingTopY, wingBottomZ );
    
    // -y
    glNormal3f( -1.0, 0.0, 0.0 ); // Normal vector.
    SubdivideAndDrawQuad( 24, 24,  0.0, 0.0, wingMidX, wingBottomY, wingTopZ,
                                  1.0, 0.0, wingBottomX, wingBottomY, wingTopZ,
                                  1.0, 1.0, wingBottomX, wingBottomY, wingBottomZ,
                                  0.0, 1.0, wingMidX, wingBottomY, wingBottomZ );
    ///////////////////////////////////////////////////////////////////////////
    // Outer Part
    ///////////////////////////////////////////////////////////////////////////
    // +z
    glNormal3f( 0.0, 0.0, 1.0 ); // Normal vector.
    SubdivideAndDrawQuad( 12, 12,  0.0, 0.0, wingTopX, wingOuterY, wingMidZ,
                                  1.0, 0.0, wingMidX, wingMidY, wingTopZ,
                                  1.0, 1.0, wingMidX, wingBottomY, wingTopZ,
                                  0.0, 1.0, wingTopX, wingBottomY, wingMidZ );
    
    // -z
    glNormal3f( 0.0, 0.0, -1.0 ); // Normal vector.
    SubdivideAndDrawQuad( 12, 12,  0.0, 0.0, wingTopX, wingOuterY, wingBottomZ,
                                  1.0, 0.0, wingTopX, wingBottomY, wingBottomZ,
                                  1.0, 1.0, wingMidX, wingBottomY, wingBottomZ,
                                  0.0, 1.0, wingMidX, wingMidY, wingBottomZ );
    
    // +x
    SubdivideAndDrawQuad( 12, 12,  0.0, 0.0, wingTopX, wingOuterY, wingMidZ,
                                  1.0, 0.0, wingTopX, wingBottomY, wingMidZ,
                                  1.0, 1.0, wingTopX, wingBottomY, wingBottomZ,
                                  0.0, 1.0, wingTopX, wingOuterY, wingBottomZ );
    
    // +y
    SubdivideAndDrawQuad( 12, 12,  0.0, 0.0, wingMidX, wingMidY, wingTopZ,
                                  1.0, 0.0, wingTopX, wingOuterY, wingMidZ,
                                  1.0, 1.0, wingTopX, wingOuterY, wingBottomZ,
                                  0.0, 1.0, wingMidX, wingMidY, wingBottomZ );
    
    // -y
    SubdivideAndDrawQuad( 12, 12,  0.0, 0.0, wingTopX, wingBottomY, wingMidZ,
                                  1.0, 0.0, wingMidX, wingBottomY, wingTopZ,
                                  1.0, 1.0, wingMidX, wingBottomY, wingBottomZ,
                                  0.0, 1.0, wingTopX, wingBottomY, wingBottomZ );
...
void DrawLeftWing()
{
    ...
    // -y
    SubdivideAndDrawQuad( 12, 12,  0.0, 0.0, -wingMidX, wingBottomY, wingTopZ,
                                   0.0, 1.0, -wingTopX, wingBottomY, wingMidZ,
                                   1.0, 1.0, -wingTopX, wingBottomY, wingBottomZ,
                                   1.0, 0.0, -wingMidX, wingBottomY, wingBottomZ);
}
...
void DrawPilot(void) {
    ...
    // In +z direction.
    glNormal3f( 0.0, 0.0, 1.0 ); // Normal vector.
    SubdivideAndDrawQuad( 24, 24,  0.0, 0.0, -pilotBackWidth, pilotMidY, pilotTopZ,
                                  1.0, 0.0, pilotBackWidth, pilotMidY, pilotTopZ,
                                  1.0, 1.0, pilotFrontWidth, pilotTopY, pilotMidZ,
                                  0.0, 1.0, -pilotFrontWidth, pilotTopY, pilotMidZ );
    
    // In +x direction
    glNormal3f( 1.0, 0.0, 0.0 ); // Normal vector.
    SubdivideAndDrawQuad( 24, 24,  0.0, 0.0, pilotFrontWidth, pilotTopY, pilotMidZ,
                                  1.0, 0.0, pilotBackWidth, pilotMidY, pilotTopZ,
                                  1.0, 1.0, pilotBackWidth, pilotMidY, pilotMidZ,
                                  0.0, 1.0, pilotFrontWidth, pilotTopY, pilotMidZ );
    
    // In -x direction
    glNormal3f( -1.0, 0.0, 1.0 ); // Normal vector.
    SubdivideAndDrawQuad( 24, 24,  0.0, 0.0, -pilotBackWidth, pilotMidY, pilotTopZ,
                                  1.0, 0.0, -pilotFrontWidth, pilotTopY, pilotMidZ,
                                  1.0, 1.0, -pilotFrontWidth, pilotTopY, pilotMidZ,
                                  0.0, 1.0, -pilotBackWidth, pilotMidY, pilotMidZ );
}
...
void DrawPlaneBody(void) {
    ...
    // In +x direction.
    glNormal3f( 1.0, 0.0, 0.0 ); // Normal vector.
    SubdivideAndDrawQuad( 24, 24,  0.0, 0.0, bodyBackWidth, bodyMidY, bodyTopZ,
                                  1.0, 0.0, bodyBackWidth, bodyBottomY, bodyMidZ,
                                  1.0, 1.0, bodyBackWidth, bodyBottomY, bodyBottomZ,
                                  0.0, 1.0, bodyBackWidth, bodyMidY, bodyBottomZ );
    // In -x direction.
    glNormal3f( -1.0, 0.0, 0.0 ); // Normal vector.
    SubdivideAndDrawQuad( 24, 24,  0.0, 0.0, -bodyBackWidth, bodyMidY, bodyTopZ,
                                  1.0, 0.0, -bodyBackWidth, bodyMidY, bodyBottomZ,
                                  1.0, 1.0, -bodyBackWidth, bodyBottomY, bodyMidZ,
                                  0.0, 1.0, -bodyBackWidth, bodyBottomY, bodyMidZ );
    
    // In +z direction
    glNormal3f( 0.0, 0.0, 1.0 ); // Normal vector.
    SubdivideAndDrawQuad( 24, 24,  0.0, 0.0, bodyBackWidth, bodyMidY, bodyTopZ,
                                  1.0, 0.0, -bodyBackWidth, bodyMidY, bodyTopZ,
                                  1.0, 1.0, -bodyBackWidth, bodyBottomY, bodyMidZ,
                                  0.0, 1.0, bodyBackWidth, bodyBottomY, bodyMidZ );
    // In -z direction
    glNormal3f( 0.0, 0.0, -1.0 ); // Normal vector.
    SubdivideAndDrawQuad( 24, 24,  0.0, 0.0, bodyBackWidth, bodyMidY, bodyBottomZ,
                                  1.0, 0.0, bodyBackWidth, bodyBottomY, bodyBottomZ,
                                  1.0, 1.0, -bodyBackWidth, bodyBottomY, bodyBottomZ,
                                  0.0, 1.0, -bodyBackWidth, bodyMidY, bodyBottomZ );
    
    // -y
    glNormal3f( 0.0, -1.0, 0.0 ); // Normal vector.
    SubdivideAndDrawQuad( 24, 24,  0.0, 0.0, bodyBackWidth, bodyBottomY, bodyMidZ,
                                  1.0, 0.0, -bodyBackWidth, bodyBottomY, bodyMidZ,
                                  1.0, 1.0, -bodyBackWidth, bodyBottomY, bodyBottomZ,
                                  0.0, 1.0, bodyBackWidth, bodyBottomY, bodyBottomZ );
    ...
    // In -z direction.
    glNormal3f( 0.0, 0.0, -1.0 ); // Normal vector.
    SubdivideAndDrawQuad( 24, 24,  0.0, 0.0, -bodyFrontWidth, bodyTopY, bodyBottomZ,
                                  1.0, 0.0, bodyFrontWidth, bodyTopY, bodyBottomZ,
                                  1.0, 1.0, bodyBackWidth, bodyMidY, bodyBottomZ,
                                  0.0, 1.0, -bodyBackWidth, bodyMidY, bodyBottomZ );
    // In +x direction
    glNormal3f( 1.0, 0.0, 0.0 ); // Normal vector.
    SubdivideAndDrawQuad( 24, 24,  0.0, 0.0, bodyFrontWidth, bodyTopY, bodyMidZ,
                                  1.0, 0.0, bodyBackWidth, bodyMidY, bodyMidZ,
                                  1.0, 1.0, bodyBackWidth, bodyMidY, bodyBottomZ,
                                  0.0, 1.0, bodyFrontWidth, bodyTopY, bodyBottomZ );
    
    // In -x direction
    glNormal3f( -1.0, 0.0, 1.0 ); // Normal vector.
    SubdivideAndDrawQuad( 24, 24,  0.0, 0.0, -bodyBackWidth, bodyMidY, bodyMidZ,
                                  1.0, 0.0, -bodyFrontWidth, bodyTopY, bodyMidZ,
                                  1.0, 1.0, -bodyFrontWidth, bodyTopY, bodyBottomZ,
                                  0.0, 1.0, -bodyBackWidth, bodyMidY, bodyBottomZ );
    ...
    SubdivideAndDrawQuad( 24, 24,  0.0, 0.0, -bodyFrontWidth, bodyTopY, bodyMidZ,
                                  1.0, 0.0, bodyFrontWidth, bodyTopY, bodyMidZ,
                                  1.0, 1.0, bodyOuterWidth, bodyOuterY, bodyBottomZ,
                                  0.0, 1.0, -bodyOuterWidth, bodyOuterY, bodyBottomZ );
    
    // In -z direction.
    glNormal3f( 0.0, 0.0, -1.0 ); // Normal vector.
    SubdivideAndDrawQuad( 24, 24,  0.0, 0.0, -bodyOuterWidth, bodyOuterY, bodyBottomZ,
                                  1.0, 0.0, bodyOuterWidth, bodyOuterY, bodyBottomZ,
                                  1.0, 1.0, bodyFrontWidth, bodyTopY, bodyBottomZ,
                                  0.0, 1.0, -bodyFrontWidth, bodyTopY, bodyBottomZ );
...
void DrawFighterPlane( void )
{
    ...
    glBindTexture( GL_TEXTURE_2D, rustTexObj );  // Texture object ID == 0 means no texture mapping.

    glRotated(-15, 0, 0 ,1);
    // Main Body
    glPushMatrix();
    glTranslated( xPlaneLocation, yPlaneLocation, planeHeight  );
    DrawPlaneBody();
    glPopMatrix();
    
    // lower left wing
    glPushMatrix();
    glTranslated( xPlaneLocation, yPlaneLocation, planeHeight + lowerWingHeight);
    glRotated(-15, 0, 1, 0);
    DrawLeftWing();
    glPopMatrix();
    ...
    // upper left wing
    glPushMatrix();
    glTranslated( xPlaneLocation, yPlaneLocation, planeHeight + upperWingHeight );
    glRotated(15, 0, 1, 0);
    DrawLeftWing();
    glPopMatrix();
    ...
    // Pilot
    glBindTexture( GL_TEXTURE_2D, windowTexObj );  // Texture object ID == 0 means no texture mapping.
    glPushMatrix();
    glTranslated( xPlaneLocation, yPlaneLocation, planeHeight );
    DrawPilot();
    glPopMatrix();
...
// Tabletop.
    ...
    // WRITE YOUR CODE HERE.
    //****************************
    // I modified the bottom part for it to show upwards

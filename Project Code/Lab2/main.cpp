...
/////////////////////////////////////////////////////////////////////////////
#define BOX_HEIGHT                CAR_HEIGHT / 2 // Height of bottom/top box
#define BOX_TOP_LENGTH         CAR_LENGTH / 2 // Length of top box
#define TYRE_OUTER_RADIUS         CAR_HEIGHT / 7 // Outer radius of tyre.
#define TYRE_INNER_RADIUS         TYRE_OUTER_RADIUS / 2 // Inner radius of tyre.
#define TYRE_X_POSITION           CAR_LENGTH / 4 // x - coordinate of tyre.
#define TYRE_Y_POSITION           CAR_WIDTH / 2 // y - coordinate of tyre.
#define TYRE_SIDES                20 // Number of sides in the rings of the Torus.
#define TYRE_RINGS                20 // Number of rings in the Torus.
#define DEGREE_TO_RADIAN          PI / 180 // Multiply this to degree to get the radian.
...
int winHeight = 800;    // Window height in pixels.
...
// Draws the bottom part of the car.
void DrawBottomBox() {
    // Bottom box
    glPushMatrix();
    // Translate it to the top of the z-axis
    glTranslated(0, 0, CAR_HEIGHT/4);
    // Scale it to actual dimensions
    glScaled(CAR_LENGTH, CAR_WIDTH, BOX_HEIGHT);
    // Draw Cube
    glutSolidCube(1);
    glPopMatrix();
}

// Draws the top part of the car.
void DrawTopBox() {
    // Top box
    glPushMatrix();
    // Translate it to the top of the bottom box
    glTranslated(0, 0, BOX_HEIGHT);
    // Scale it to actual dimensions
    glScaled(BOX_TOP_LENGTH, CAR_WIDTH, BOX_HEIGHT);
    // Draw Cube
    glutSolidCube(1);
    glPopMatrix();
}

// Draws the front and back tyres
void DrawFourWheels() {
    for (int i = 0; i < 4; i++) {
        glPushMatrix();
        double tyre_x = TYRE_X_POSITION;
        double tyre_y = TYRE_Y_POSITION;
        ...
        glTranslated(tyre_x, tyre_y, TYRE_OUTER_RADIUS);
        
        // Tyres are all identical
        glRotated(90, 1, 0, 0);
        glutSolidTorus(TYRE_INNER_RADIUS, TYRE_OUTER_RADIUS, TYRE_SIDES, TYRE_RINGS);
        glPopMatrix();
    }
}
/////////////////////////////////////////////////////////////////////////////
...
void DrawAllCars( void ) 
{
    ...
    {
        //***********************************************************************
        CarType& currentCar = car[i];
        glPushMatrix();
        // Aligns the car to its great Circle
        glRotated(currentCar.rotAngle, currentCar.xzAxis[0], 0, currentCar.xzAxis[1]);
        // Movement around its great Circle
        glRotated(currentCar.angularPos, 0, 1, 0);
        glTranslated(0, 0, PLANET_RADIUS);
        DrawOneCar(currentCar.bodyColor);
        glPopMatrix();
        //***********************************************************************
    }
}
...
void MyDisplay( void )
{
    ...
    glLoadIdentity();
    //***********************************************************************
    int NEAR = EYE_INIT_DIST - CLIP_PLANE_DIST;
    int FAR =  EYE_INIT_DIST + CLIP_PLANE_DIST;
    gluPerspective( VERT_FOV, (double)winWidth / winHeight, NEAR, FAR);
    //***********************************************************************
    ...
    //***********************************************************************
    float camX = eyeDistance * sinf(eyeLongitude * DEGREE_TO_RADIAN) * cosf(eyeLatitude * DEGREE_TO_RADIAN);
    float camY = eyeDistance * sinf(eyeLatitude * DEGREE_TO_RADIAN);
    float camZ = eyeDistance * cosf(eyeLongitude * DEGREE_TO_RADIAN) * cosf(eyeLatitude * DEGREE_TO_RADIAN);
    // Camera Position
...
int main( int argc, char** argv )
{
    ...
    // For the first glutTimerFunc call, I put on a delay
    // so the program can enter the main loop before the call
